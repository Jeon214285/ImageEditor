...
def test_face_detect_ui(page: Page):
    ...
    # 탐지 버튼 클릭 및 로딩시간 고려
    with page.expect_response("**/api/detect/face") as response_info:
...
def test_plate_detect_ui(page: Page):
    # 이미지 불러오기(이미지가 있어야 실행되기 때문)
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", test_image_path)
    page.wait_for_timeout(500)
    ...
    # 탐지 버튼 클릭 및 로딩시간 고려
    with page.expect_response("**/api/detect/plate") as response_info:
        ...
        # 버튼 비활성화 확인
        expect(page.get_by_role("button", name="차량 번호판 탐지")).to_be_disabled()
    ...
    # 버튼 돌아왔는지 확인
    expect(page.locator("body")).to_have_css("cursor", "default")
    expect(page.get_by_role("button", name="차량 번호판 탐지")).to_be_enabled()
...
def test_face_detect(page: Page):
    ...
    page.route("**/api/detect/face", stub_success_response)
...
def test_plate_detect(page: Page):
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", "tests/test.png")
    page.wait_for_timeout(500)
    ...
    page.route("**/api/detect/plate", stub_success_response)
...
def test_face_no_detect(page: Page):
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", test_image_path)
    page.wait_for_timeout(500)
    ...
    # 탐지 실패 상황
    page.route("**/api/detect/face", lambda route: route.fulfill(
        status=200,
        content_type="application/json",
...
def test_plate_no_detect(page: Page):
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", test_image_path)
    page.wait_for_timeout(500)
    ...
    # 탐지 실패 상황
    page.route("**/api/detect/plate", lambda route: route.fulfill(
        status=200,
        content_type="application/json",
        body=json.dumps([]) # 빈 배열 반환
    ))
...
def test_face_detect_error(page: Page, status_code, expected_alert):
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", test_image_path)
    page.wait_for_timeout(500)

    # error 검증
    page.route("**/api/detect/face", lambda route: route.fulfill(status=status_code))
...
def test_plate_detect_error(page: Page, status_code, expected_alert):
    page.goto("http://localhost:8000")
    page.set_input_files("#imageInput", test_image_path)
    page.wait_for_timeout(500)

    # error 검증
    page.route("**/api/detect/plate", lambda route: route.fulfill(status=status_code))
